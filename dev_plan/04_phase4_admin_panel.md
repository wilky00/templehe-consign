...
- **AppConfig key registry shipped (ADR-018, branch `phase4-prework`).** `services/app_config_registry.py` enumerates every `app_config` key with `KeySpec(name, category, field_type, description, default, parser, serializer, validator)`. Phase 4 admin form for global variables reads `all_specs()` to render inputs and writes through `set_typed()` for per-key validation. The YAML-seed loader (`scripts/seed_config.py`) MUST also call `set_typed()` so YAML and UI enforce the same validators.
- **Equipment status registry shipped (ADR-018).** `services/equipment_status_machine.py` owns the canonical status enum + per-status metadata + denylisted transitions. Phase 4 admin "manually transition this record" UI reads `all_status_values()` for the dropdown and `is_forbidden_transition()` for the gate. DB CHECK constraint on `equipment_records.status` (migration 013) blocks unknown writes at the data layer.
- **Inspection prompt + red-flag rule versioning shipped (ADR-018).** `category_inspection_prompts` and `category_red_flag_rules` carry `version` + `replaced_at` columns (migration 014). Phase 4 admin's "edit a prompt / red-flag rule" path is `category_versioning_service.supersede_*(existing, new_*=...)` — inserts a new row + flips `replaced_at` on the old; never UPDATE in place. Phase 5 iOS writers MUST embed `prompt_version` / `rule_version` in the JSONB blobs they author so Phase 7 PDF reports stay historically accurate.

---

## Architectural Debt to Address

The Phase 3 → Phase 4 architecture review (project_notes/decisions.md ADR-018) shipped four pre-work fixes (multi-role users, status machine, AppConfig registry, prompt versioning); the items below were intentionally deferred into Phase 4 because the natural fix lives inside Phase 4's surface area. Each one needs a Phase-4-sized motion (UI + backend + tests), not a Phase 4 *afterthought*.

- **Notification template registry (Critical #2, deferred).** Today every notification subject + body is composed inline across `equipment_status_service`, `routers/record_locks`, `auth_service`. Phase 4 admin "edit email copy" needs a template registry: `services/notification_templates.py` registers `Template(name, subject_renderer, body_renderer, channel, variables)`; existing inline composers migrate to `templates.render(name, ctx)`. Coordinate with the AppConfig registry pattern — same admin-form rendering shape.
- **Configurable read-only roles (#5).** `notification_preferences_service._READ_ONLY_ROLES = frozenset({"customer"})` is hardcoded; the visibility flag is configurable. Add a sibling AppConfig key `notification_preferences_read_only_roles` with the same `{"roles": [...]}` shape so admin can flip both gates.
- **`lead_routing_rules.priority` uniqueness (#7).** Add UNIQUE on `(rule_type, priority)` and a renumber-helper for the admin's drag-to-reorder UX. Migration is small; the UX work is the bulk.
- **Lead routing rule JSON Schema (#8).** `_validate_conditions` is a Python function. Phase 4 admin form needs server-driven validation. Pydantic models per `rule_type`, exposed via OpenAPI, drive both the validator and the form generator.
- **Two-prefs reconciliation (#9).** `customers.communication_prefs` (per-event opt-in/out) vs `notification_preferences` (channel choice). Phase 4 admin "edit a customer's notifications" should surface a unified read view + clearly route writes to the right table. No data merge — keep tables — but add a unified read endpoint.
- **Lock resource registry (#10).** `record_locks.record_type` whitelist sits in `routers/record_locks.py` and only knows `equipment_record`. Phase 4 will likely lock customers, calendar events, routing rules. Strategy pattern: each lockable resource registers a `LockableResource(type, display_name, audit_prefix, reference_loader)`.
- **CalendarEvent multi-attendee (#11).** New `calendar_event_attendees` join table. Phase 4 admin scheduling UX surfaces "who's attending" naturally; backend gains a list of users per event.
- **Nullable `customers.user_id` (#12).** Admin needs to create walk-in customers without first inviting a user account. Schema migration + new `customers.invite_email` field for the deferred portal invite.
- **Equipment record assignment watchers (#13).** Add `equipment_record_watchers` join table for secondary assignees / mentors. Notification dispatch widens to include watchers.
- **Equipment category versioning (#14).** Same versioning pattern Phase 4 pre-work applied to prompts + red-flag rules; extend to the `equipment_categories` row itself so renaming a category doesn't rewrite history.
- **Round-robin counter on Redis (#15).** Currently a Postgres row-lock UPDATE. Hot row under high concurrency. Swap to Redis `INCR` at GCP migration time (already noted in lead_routing_service docstring).
- **`notification_jobs.template` registry (#16).** Resolved as a side-effect of the notification template registry above — once templates are first-class, the `template` column gains metadata an admin "view failed jobs" UI can render against.
