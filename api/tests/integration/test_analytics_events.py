...
@pytest.mark.asyncio
async def test_analytics_event_recorded_for_anonymous(client: AsyncClient, db_session: AsyncSession):
...
@pytest.mark.asyncio
async def test_analytics_event_rejects_pii_in_metadata(client: AsyncClient, db_session: AsyncSession):
    await db_session.commit()
    r = await client.post(
        "/api/v1/analytics/event",
        json={
